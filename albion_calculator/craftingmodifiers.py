...
def _load_crafting_modifiers():
    raw_data = _load_crafting_modifiers_file()
    crafting_modifiers = {}
    for location in raw_data:
        city = _CLUSTER_ID.get(location.get('@clusterid', None), None)
        if city is None:
            continue
        crafting_modifiers[city] = {_replace_refining_category(modifier['@name']): float(modifier['@value'])
                                    for modifier in location['craftingmodifier']}
    return crafting_modifiers


def _get_return_rate(city, item_category, use_focus=False):
    focus_bonus = 0.59 if use_focus else 0
    local_crafting_bonus = _crafting_bonus[city].get(item_category, 0) + _BASE_CRAFTING_BONUS + focus_bonus
    return round(1 - 1 / (1 + local_crafting_bonus), 3)


def _replace_refining_category(name):
    return _SUBCATEGORY_REPLACEMENTS.get(name, name)


def _load_crafting_modifiers_file():

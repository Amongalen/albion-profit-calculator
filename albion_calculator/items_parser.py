...
from albion_calculator.items import Item, RecipeType, Ingredient, Recipe
...
def extract_recipes(item: dict) -> list[Recipe]:
    ...
    transport_recipe = [Recipe(item_id, RecipeType.TRANSPORT, ingredients=[Ingredient(item_id, 1, 0)])]
...
def _extract_single_recipe_details(recipe_data: dict, item: dict, is_upgrade_recipe: bool) -> Optional[Recipe]:
    ...
    recipe_type = RecipeType.UPGRADE if is_upgrade_recipe else RecipeType.CRAFTING
